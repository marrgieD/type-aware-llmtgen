...
def parse_args():
    ...
    parser.add_argument("--lr", type=float, default=5e-5,
                       help="Learning rate")
    parser.add_argument("--lr_nodp", type=float, default=None,
                       help="Learning rate for non-DP training (overrides --lr when privacy is disabled)")
    ...
    parser.add_argument("--log_timing_steps", type=int, default=0,
                       help="If >0, log per-step timing every N steps (uses torch.cuda.synchronize).")
    parser.add_argument("--disable_nodp_clip", action="store_true",
                       help="Disable grad clipping in non-DP mode (for ablation).")
...
def train_step(model, optimizer, batch, args, trainable_params: List[torch.nn.Parameter], logger, global_step: int):
    """
    One optimization step.
    - DP path: relies on Opacus for clipping/noise; keeps grad_sample check
    - Non-DP path: explicitly clips gradients to mimic DP stability
    """
    # do_timing = (args.log_timing_steps is not None) and (args.log_timing_steps > 0) and (global_step % args.log_timing_steps == 0)
    # if do_timing and args.device.startswith("cuda"):
    ...
    #     torch.cuda.synchronize()
    # t2 = time.time()
    # Non-DP: add clipping to align dynamics with DP path
    if (not args.enable_privacy) and (not args.disable_nodp_clip):
       torch.nn.utils.clip_grad_norm_(trainable_params, args.max_grad_norm)
    # if do_timing and args.device.startswith("cuda"):
    #     torch.cuda.synchronize()
    # t3 = time.time()
    # DP-only: strict grad_sample check (Opacus)
    ...
    # if do_timing:
    #     # fwd: model forward, loss: compute_loss, bwd: backward, clip: clip_grad_norm_, step: optimizer.step
    #     logger.info(
    #         f"[timing step={global_step}] "
    #         f"fwd={t1-t0:.3f}s loss={t1b-t1:.3f}s bwd={t2-t1b:.3f}s "
    #         f"clip={t3-t2:.3f}s step={t4-t3:.3f}s total={t4-t0:.3f}s"
    #     )

    return loss.detach().item(), lm_loss_item, expert_loss_item
import warnings
...
def main():
    ...
    if (not args.enable_privacy) and (args.lr_nodp is not None):
        effective_lr = args.lr_nodp
    optimizer = torch.optim.AdamW(trainable_params, lr=effective_lr)
    logger.info(f"Using lr={effective_lr} (enable_privacy={args.enable_privacy})")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        try:
            for step, batch in enumerate(data_iter):
                if global_step >= args.max_train_steps:
                    break
                # ===== progress accounting (AFTER BatchMemoryManager) =====
                bs = batch["input_ids"].size(0)
                seen_samples += bs

                # 建议用一个独立的计数器，不要依赖 global_step（因为DP时global_step可能是update-step或micro-step）
                if (step % args.logging_steps) == 0:
                    logger.info(
                        f"[progress] epoch={epoch} iter={step} bs={bs} seen_samples={seen_samples}"
                    )
                ...
                loss_val, lm_loss_item, expert_loss_item = train_step(
                    model=model,
                    optimizer=optimizer,
                    batch=batch,
                    args=args,
