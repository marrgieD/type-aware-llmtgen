...
def parse_args():
    ...
    parser.add_argument("--expert_loss_weight", type=float, default=0.5,
                       help="Weight for expert loss in hybrid loss")
    parser.add_argument("--lm_loss_weight", type=float, default=2.0, 
...
import warnings
@torch.no_grad()
def evaluate(model, dataloader, device, tokenizer, target_col_name=None, dataset_metadata=None):
    """
    运行 Validation 并计算关键指标：
    1. Val LM Loss & PPL (整体语言能力)
    2. Val Expert Loss (条件拟合能力)
    ...
    # 确定 Target Column 在 Expert 中的索引 (如果提供了 metadata 和 name)
    # 这是一个简化的查找逻辑，假设 metadata 顺序和 expert 输出顺序一致
    target_col_idx = -1
    if target_col_name and dataset_metadata:
        cat_cols = [c for c, m in dataset_metadata.items() if m['type'] == 'categorical']
        if target_col_name in cat_cols:
            target_col_idx = cat_cols.index(target_col_name)
    ...
    for batch in tqdm(dataloader, desc="Validating", leave=False):
        ...
        # 1. 计算 Loss
        _, lm_loss_item, expert_loss_item = compute_loss(
            outputs, batch, expert_loss_weight=1.0, lm_loss_weight=1.0 # 验证时权重 1:1 即可，只看数值
        )
        total_lm_loss += lm_loss_item
        total_expert_loss += expert_loss_item
        total_steps += 1
        
        # 2. 监控 Target UNK / Collapse (LM Head 预测)
        # 我们看 LM Head 在预测下一个 token 时，是不是倾向于输出 UNK
        lm_logits = outputs["lm_logits"]
        preds = torch.argmax(lm_logits, dim=-1) # [Batch, Seq]
        
        # 简单统计：所有预测中 UNK 的比例 (作为 Collapse 的预警)
        # 如果你想更精确地只看 Target 列，需要复杂的 Mask，这里用全局 UNK 率作为近似
        mask = (batch["labels"] != -100)
        valid_preds = preds[mask]
        target_unk_count += (valid_preds == unk_id).sum().item()
        target_total_count += valid_preds.numel()
    ...
    unk_rate = target_unk_count / (target_total_count + 1e-8)
    expert_acc = expert_correct / (expert_total + 1e-8)
...
def main():
    ...
    if args.validation_file:
        ...
        validation_dataset.max_length = args.max_length
    ...
    lora_config = LoraConfig(
        ...
        modules_to_save=["num_expert", "mixed_expert", "cat_expert", "lm_head", "wte"],
    ...
    # 准备 Validation Dataloader (如果有)
    val_dataloader = None
    if validation_dataset:
        val_dataloader = DataLoader(
            validation_dataset,
            batch_size=args.batch_size, # 验证可以用大一点的 batch
            shuffle=False,
            collate_fn=make_collate_fn(tokenizer),
            drop_last=False
        )
            ...
            for step, batch in enumerate(data_iter):
                ...
                if global_step % args.logging_steps == 0:
                    ...
                    logger.info(
                        ...
                        f"Loss={loss.item():.4f} | "
                        f"LM_Loss={lm_loss_item:.4f} | "
                        f"Exp_Loss={expert_loss_item:.4f} | "
                        f"LR={current_lr:.2e}"
                    )
                if val_dataloader and (global_step % eval_steps == 0):
                    ...
                    logger.info(
                        ...
                        f"Val_PPL={metrics['val_ppl']:.2f} | "    # PPL: Joint崩没崩
                        f"Val_Exp_Loss={metrics['val_expert_loss']:.4f} | "
                    ...
                    if metrics['val_ppl'] > 100: # 阈值可调
                        logger.warning(f"⚠️ Warning: Perplexity is exploding ({metrics['val_ppl']:.2f}). Check LR or weights.")
